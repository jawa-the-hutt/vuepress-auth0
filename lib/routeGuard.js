  ...
  return async (to, from, next) => {
    ...
    if (linkIndex > -1) {
      ...
      if ((navConfig.meta && navConfig.meta.auth) || options.authAll) {
        ...
        if(!options.authAll && navConfig.meta.roles && navConfig.meta.roles.length > 0 && profileRoles.length > 0) {
        ...
        if(options.authAll && options.authAllRoles.length > 0 && profileRoles.length > 0) {
            const matchingRoles = [...new Set(options.authAllRoles)].filter(x => new Set(profileRoles).has(x));
            userHasMatchingRoles = matchingRoles.length > 0 ? true : false;
        }
